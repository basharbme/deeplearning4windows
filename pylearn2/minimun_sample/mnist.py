...
class TestDataset(dense_design_matrix.DenseDesignMatrix):
    """
    The MNIST dataset
    ...
    def __init__(self, which_set, center=False, shuffle=False,
        ...
        print "y:%s" % y # => [[5], [0], [4], ....]
        print "y size:%d" % y.size # => 60000
        print "axes:%s" % axes # => ['b', 0, 1, 'c']
        print "labels:%s" % y_labels # => 10 （教師信号のラベル数）
        print "topo_view:%s" % topo_view # 入力データのndArray？
